...
from app.errors.exceptions import BadRequestError, ConflictError
...
def register(data: RegisterIn):
    ...
    try:
        user = crud_user.create_user(
            str(data.email).strip().lower(), ph, data.full_name, data.nim
        )
    except Exception as exc:
        if is_unique_violation(exc):
            raise ConflictError(detail="Email sudah terdaftar") from exc
        raise
...
def login(data: LoginIn, resp: Response):
    user = crud_user.get_user_by_email(str(data.email).strip().lower())
    if not user:
